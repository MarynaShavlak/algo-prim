...
Кадри малюють ті самі функції зі :mod:`prim_mst.visualization`
(``draw_prim_step``, ``draw_cut``, ``draw_cable_plan`` тощо) — цей модуль лише
«зшиває» готові фігури в один зациклений GIF через Pillow, а за можливості
додатково кодує ту саму послідовність кадрів у **MP4** (через ``ffmpeg``).

Чому окремий модуль: :mod:`prim_mst.core` лишається чистим алгоритмом,
:mod:`prim_mst.visualization` відповідає за статичні рисунки, а збірка
...
з'являються лише на пізніх кадрах (зелені ребра дерева, червоні акценти), не
загубилися.
...
def _shared_palette(frames: Sequence[Image.Image], colors: int) -> Image.Image:
    ...
    Зелені (дерево) та червоні (акцент кроку) пікселі з'являються лише на
    частині кадрів; якщо взяти палітру з одного кадру, ці кольори можуть
